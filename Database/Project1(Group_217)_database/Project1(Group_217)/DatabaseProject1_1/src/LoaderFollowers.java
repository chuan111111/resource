...
public class LoaderFollowers {
    ...
    public static void main(String[] args) {
        ...
        try {
            stmt = con.prepareStatement("INSERT INTO public.followers (Followers, Followed) " +
                    "VALUES (?,?)" + " ON CONFLICT (Followers, Followed) DO NOTHING;");
        } catch (SQLException e) {
        ...
        for (Post p: posts) {
            if (con != null) {
                try {
                    for (String s: p.getAuthorFollowedBy()) {
                        stmt.setString(1, p.getAuthor());
                        stmt.setString(2, s);
                        stmt.executeUpdate();
                        cnt++;
                        if (cnt % 1000 == 0) {
