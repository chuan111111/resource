...
public class MainPanel extends JPanel implements KeyListener,subject<Ball> {
    ...
    public void registerObserver(Ball ball) {
        if (observerBallList.get(0) == null){
            observerBallList.remove(0);
        }
    ...
    @Override
    public void notifyObservers(Ball whiteBall) {
        for (Ball b:observerBallList
             ) {
    ...
    public void restartGame() {
        ...
        this.observerBallList=new ArrayList<>();//添加白球
        this.observerBallList.add(whiteBall);
  ...
  //      observerBallList.forEach(o->o.update(keyChar));
