...
def generate_directory_html(directory_path):
    ...
    if len(directory_path.split('/')) > 1:
        user = directory_path.split('/')[1]
        print("这里是" + user)
        if len(user) != 0:
            pass0 = user_password.get(user)
            print(pass0)
            user = user + ":" + pass0
            user = base64.b64encode(user.encode()).decode('utf-8')
            print(user)
    else: user="Y2xpZW50MToxMjM="
        # author=user+"="+pass0
    # 生成文件列表
    ...
    html_content = f'''
        ...
        <body>
            ...
            <form id="uploadForm" enctype="multipart/form-data" onsubmit="event.preventDefault(); uploadFile()">
                ...
                <input type="text" name="uploadPath" id="uploadPath" placeholder="Enter upload path">
                <br>
                <label for="fileInput">Select File:</label>
                <input type="file" name="file" id="fileInput" accept=".txt, .jpg, .png, .zip, .pdf, .py">
                <br>
            ...
            <script>
                function uploadFile() {{
                    ...
                    if (file && uploadPath) {{
                        ...
                        const firstPartOfPath = encodeURIComponent(uploadPath.split('/')[0]);
                        // Use pathlib.Path to construct a safe URL
                        const encodedPath = encodeURIComponent(uploadPath);
                        const url = new URL(`http://localhost:8080/upload`);
                        ...
                        alert('Please select a file and enter the upload path.');
                    }}
                }}
        ...
        # 有path=
            ...
            # =后面不空
                ...
                if not os.path.exists('data/' + upload_user_dir):
                    ...
                    return client_post(client_socket, response_header, response_first_line, upload_user,
                                       upload_user_dir, request_header, file_content)
    ...
    if os.path.exists(file_path):
        if os.path.isdir(file_path):
            ...
            if chunked:
                ...
                return set_response(response, mimetypes.guess_type(file_path)[0] or 'application/octet-stream',
                                    response_header)
    else:
        print("error")
        response_first_line[0] = 'HTTP/1.1 404 Not Found\r\n'
...
def client_post(client_socket, response_header, response_first_line, upload_user, upload_user_dir, request_header,
                file_content):
        ...
        if request:
            ...
            if match:
                ...
                if match == 'upload':
                    ...
                    con_to_send = checkup(path, request_header, client_socket, response_header, response_first_line,
                                          method, username, file_content)
